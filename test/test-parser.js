...
test('string works', t => {
  t.throws(() => string('foo').tryParse(''));
  t.throws(() => string('foo').tryParse('f'));
  t.throws(() => string('foo').tryParse('fo'));
  t.throws(() => string('foo').tryParse('boo'));
  t.throws(() => string('foo').tryParse('xfoo'));

  t.is(string('foo').tryParse('foo'), 'foo');
  t.is(string('foo').tryParse('foobar'), 'foo');
  t.is(string('').tryParse('foobar'), '');
  t.is(string('').tryParse(''), '');
});
...
test('Parser#many', t => {
  ...
  t.deepEqual(one.many().parse([2]).result.value, []);

  t.deepEqual(one.many().parse([1, 2]).result.value, ['1']);

  t.deepEqual(one.many().parse([1, 1, 2]).result.value, ['1', '1']);
});
...
test('sepBy1', t => {
  ...
  t.deepEqual(one.sepBy1(comma).parse([1, ',', 1]).result.value, ['1', '1']);
  t.deepEqual(one.sepBy1(comma).parse([1, ',', 1, ',']).result.value, [
    '1',
    '1',
  ]);
  t.deepEqual(one.sepBy1(comma).parse([1, ',', 1, ',', 1]).result.value, [
    '1',
    '1',
    '1',
  ]);
...
test('sepBy', t => {
  ...
  t.deepEqual(one.sepBy(comma).parse([]).result.value, []);
  t.true(one.sepBy(comma).parse([1]).success);
  t.deepEqual(one.sepBy(comma).parse([1]).result.value, []);
  t.true(one.sepBy(comma).parse([1, 1]).success);
  t.deepEqual(one.sepBy(comma).parse([1, 1]).result.value, []);

  t.true(one.sepBy(comma).parse([1, ',', 1]).success);
  t.deepEqual(one.sepBy(comma).parse([1, ',', 1]).result.value, ['1', '1']);
  t.deepEqual(one.sepBy(comma).parse([1, ',', 1, ',']).result.value, [
    '1',
    '1',
  ]);
  t.deepEqual(one.sepBy(comma).parse([1, ',', 1, ',', 1]).result.value, [
    '1',
    '1',
    '1',
  ]);
...
test('lift2', t => {
  ...
  t.deepEqual(f(Parser.of(1), Parser.of(2)).parse([]).result.value, 3);
});
...
test('sequence', t => {
  ...
  t.deepEqual(sequence(one, two).parse([1, 2]).result.value, ['1', '2']);
  t.true(sequence(one, two).parse([1, 2, 3]).success);
  t.deepEqual(sequence(one, two).parse([1, 2, 3]).result.value, ['1', '2']);
});
...
test('alternatives', t => {
  ...
  t.deepEqual(alternatives(one, two).parse([1]).result.value, '1');
  t.deepEqual(alternatives(one, two).parse([2]).result.value, '2');
  t.deepEqual(alternatives(one, two).parse([2, 1]).result.value, '2');
});
...
test('operator', t => {
  ...
  t.is(number.operators(basicOperators).parse([1, '+', 2]).result.value, 3);
  t.is(
...
test('CSV parser', t => {
  let comma = char(',');
  let dquote = char('"');
  let textdata = satisfy(c =>
    /[\u{20}-\u{21}]|[\u{23}-\u{2B}]|[\u{2D}-\u{7E}]/u.test(c)
  );
  let nonescaped = textdata.many().map(chars => chars.join(''));

  let escaped = sequence(
    dquote,
    alternatives(dquote.then(dquote), textdata, comma)
      .many()
  ...
  let record = sequence(
    field,
    sequence(comma, field)
      .map(([c, f]) => f)
      .many()
  ).map(([f, rest]) => [f].concat(rest));

  t.deepEqual(record.parse('foo,bar').result.value, ['foo', 'bar']);

  t.deepEqual(record.parse('"foo","bar"').result.value, ['foo', 'bar']);

  t.deepEqual(record.parse('"foo","bar,baz"').result.value, ['foo', 'bar,baz']);

  t.deepEqual(record.parse('""""').result.value, ['"']);

  t.deepEqual(record.parse('"foo","bar,baz"""').result.value, [
    'foo',
    'bar,baz"',
  ]);
});
